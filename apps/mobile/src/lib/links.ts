 ...
 * The rehype plugin (rehypeLinks) has no equivalent here: MessageText splits its text leaves itself.
 */
...
export type Entity =
  ...
  /** A file or folder path, as written: it can't be opened from the phone, only copied. */
...
export function splitEntities(text: string): TextPart[] {
  ...
  for (const m of matchAll(text, ENTITY_RE)) {
    const g = m.groups ?? {};
    let found = m[0];
...
/** Classes of a link in text: HeroUI's link color (the web adds an underline on hover, which a phone doesn't have). */
export const linkClass = "text-link";
