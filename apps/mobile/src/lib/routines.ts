...
import { dateFormat, listFormat as intlList } from "./intl";

/* apps/web/src/lib/routines.ts */
...
export function weekdays(style: "long" | "short" = "long") {
  const format = dateFormat(locale, { weekday: style });
  // 2000-01-02 is a Sunday.
  return [1, 2, 3, 4, 5, 6, 0].map((d) => ({ value: d, label: format.format(new Date(2000, 0, 2 + d)) }));
}

/** "Monday, Wednesday and Friday"; Hermes may lack Intl.ListFormat, then a plain list. */
function listFormat(names: string[]) {
  if (typeof Intl.ListFormat === "function") return intlList(locale, { type: "conjunction" }).format(names);
  return names.join(", ");
}
...
export function scheduleLabel(r: Pick<Routine, "cron" | "schedule">) {
  ...
  const t = messages;
  const [hour, min] = f.time.split(":").map(Number);
  const time = dateFormat(locale, { hour: "2-digit", minute: "2-digit" }).format(new Date(2000, 0, 1, hour, min));
  if (f.mode === "daily") return t.daily(time);
  if (f.mode === "weekdays") return t.weekdays(time);
  const names = weekdays().filter((d) => f.days.includes(d.value)).map((d) => d.label);
  return t.weekly(listFormat(names), time);
}
